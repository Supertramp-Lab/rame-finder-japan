...
F: area_label_en, G: area_label_ja, H: area_label_zh, I: area_label_ko, J: area_label_zh_hant
K: lat, L: lng, M: rating, N: reviews, O: price_range
P: hours_en, Q: hours_ja, R: hours_zh, S: hours_zh_hant, T: hours_ko
U: flavors, V: vibes
W～AK: タグ（tag1～tag3）
AL～AO: コメント（comment_en～comment_ko, comment_zh_hant）
AP: mapUrl
...
AREA_META = {
    "shinjuku": {"id": "shinjuku", "name": {"en": "Shinjuku", "zh-hant": "新宿", "zh": "新宿", "ko": "신주쿠", "ja": "新宿"}, "lat": 35.6938, "lng": 139.7036},
    "shibuya": {"id": "shibuya", "name": {"en": "Shibuya", "zh-hant": "澀谷", "zh": "渋谷", "ko": "시부야", "ja": "渋谷"}, "lat": 35.6580, "lng": 139.7016},
    "ginza": {"id": "ginza", "name": {"en": "Ginza", "zh-hant": "銀座", "zh": "銀座", "ko": "긴자", "ja": "銀座"}, "lat": 35.6714, "lng": 139.7659},
}
...
def build_shop(r):
    def tag(n):
        ...
        return {
            "l": {
                ...
                "zh-hant": r.get(f"tag{n}_label_zh_hant", label_en).strip(),
    ...
    shop = {
        ...
        "area_label": {
            ...
            "zh-hant": r.get("area_label_zh_hant", "").strip(),
        ...
        "hours": {
            ...
            "zh": r.get("hours_zh", "").strip(),
            "zh-hant": r.get("hours_zh_hant", "").strip(),
            "ko": r.get("hours_ko", "").strip(),
        },
        ...
        "comment": {
            ...
            "zh-hant": r.get("comment_zh_hant", "").strip(),
...
def convert_tab(tab_name, output_path):
    ...
    print(f"  ✓ {len(shops)} 店舖を書き出しました → {output_path}")
